...
const util = (function () {
    ...
    // 定义一个名为findTargetElement的函数，用于查找指定的DOM元素
    function findTargetElement(targetContainer) {
        // 获取页面的文档对象
        const body = window.document;
        let tabContainer; // 用于存储查找到的元素
        let tryTime = 0; // 尝试查找的次数
        const maxTryTime = 120; // 最大尝试次数
        let startTimestamp; // 记录开始尝试的时间戳
        ...
        return new Promise((resolve, reject) => {
            ...
            function tryFindElement(timestamp) {
                ...
                // 如果已经过了500毫秒，开始尝试查找元素
                if (elapsedTime >= 500) {
                    GM_log("查找元素：" + targetContainer + "，第" + tryTime + "次");
                    tabContainer = body.querySelector(targetContainer);
                    if (tabContainer) {
                        resolve(tabContainer); // 如果找到元素，解析Promise
                    } else if (++tryTime === maxTryTime) {
                        reject(); // 如果超过最大尝试次数，拒绝Promise
                    } else {
                        startTimestamp = timestamp;
                    }
                }
                // 如果元素还未找到并且尝试次数未达到最大值，继续递归调用自身
    ...
    // 定义一个函数，用于检测URL变化并重新加载页面
    function urlChangeReload() {
        const oldHref = window.location.href;
        let interval = setInterval(() => {
            let newHref = window.location.href;
            // 如果URL发生变化，清除定时器并重新加载页面
            if (oldHref !== newHref) {
    ...
    function reomveVideo() {
        setInterval(() => {
            ...
            for (let video of document.getElementsByTagName("video")) {
                if (video.src) {
                    video.removeAttribute("src"); // 移除视频元素的src属性
    ...
    function syncRequest(option) {
        return new Promise((resolve, reject) => {
            option.onload = (res) => {
                resolve(res); // 如果请求成功，解析Promise
            };
            option.onerror = (err) => {
                reject(err); // 如果请求失败，拒绝Promise
            };
            GM_xmlhttpRequest(option); // 使用GM_xmlhttpRequest发起请求
        });
    }

    // 返回一个包含各种功能的对象
    return {
        req: (option) => syncRequest(option), // 发起请求的函数
        findTargetEle: (targetEle) => findTargetElement(targetEle), // 查找指定元素的函数
        urlChangeReload: () => urlChangeReload(), // 检测URL变化并重新加载页面的函数
        reomveVideo: () => reomveVideo() // 移除视频元素的函数
    }
})();
